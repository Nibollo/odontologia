...
import { toLabel } from "./utils/numbering";
export { type NumberingSystem } from "./utils/numbering";
export { type OdontogramPlugin } from "./plugin";
import { getQuadrant, LAYER_Z } from "./plugin";
const tooth11Url = "/assets/teeth-svgs/11.svg";
const tooth13Url = "/assets/teeth-svgs/13.svg";
const tooth14Url = "/assets/teeth-svgs/14.svg";
const tooth16Url = "/assets/teeth-svgs/16.svg";
const tooth14OcclUrl = "/assets/teeth-svgs/14_occl.svg";
const tooth16OcclUrl = "/assets/teeth-svgs/16_occl.svg";
/* Tooth SVG Test UI (v2) - vanilla JS */

let changeCallback: ((state: any) => void) | null = null;
export function setChangeCallback(cb: (state: any) => void) {
  changeCallback = cb;
}
function notifyChange() {
  if (changeCallback) changeCallback(getFullStatus());
}
...
async function loadInlineIcon(button: Any){
  ...
  try{
    ...
    const existingSvg = button.querySelector(".icon-svg");
    if(existingSvg) existingSvg.remove();
    const existingImg = button.querySelector(".icon-img");
    if(existingImg) existingImg.remove();
    const label = button.querySelector(".odon-btn-label");
    if(label){
      button.insertBefore(svg, label);
    }else{
...
function applyToggleA11y(btn: Any, labelKey: Any, collapsed: Any){
  ...
  btn.setAttribute("odon-title", text);
...
function updateToothTooltip(toothNo: number){
  ...
  for(const tile of tiles){
    if(text) tile.setAttribute("odon-title", text);
    else tile.removeAttribute("odon-title");
  }
}
...
function updateWarningsFromState(state: Any){
  ...
  for(const msg of warnings){
    const item = el("div", { class: "odon-warning-item", text: "⚠ " + msg });
    w.appendChild(item);
  }
}
...
function syncControlsFromState(state: Any){
  const pulpInflam = $("#pulpInflam");
  if (pulpInflam) pulpInflam.checked = !!state.pulpInflam;
  
  const endoResection = $("#endoResection");
  if (endoResection) endoResection.checked = !!state.endoResection;
  
  const fissureSealing = $("#fissureSealing");
  if (fissureSealing) fissureSealing.checked = !!state.fissureSealing;
  
  const contactMesial = $("#contactMesial");
  if (contactMesial) contactMesial.checked = !!state.contactMesial;
  
  const contactDistal = $("#contactDistal");
  if (contactDistal) contactDistal.checked = !!state.contactDistal;
  
  const bruxismWear = $("#bruxismWear");
  if (bruxismWear) bruxismWear.checked = !!state.bruxismWear;
  
  const bruxismNeckWear = $("#bruxismNeckWear");
  if (bruxismNeckWear) bruxismNeckWear.checked = !!state.bruxismNeckWear;
  
  const brokenMesial = $("#brokenMesial");
  if (brokenMesial) brokenMesial.checked = !!state.brokenMesial;
  
  const brokenIncisal = $("#brokenIncisal");
  if (brokenIncisal) brokenIncisal.checked = !!state.brokenIncisal;
  
  const brokenDistal = $("#brokenDistal");
  if (brokenDistal) brokenDistal.checked = !!state.brokenDistal;
  
  const extractionWound = $("#extractionWound");
  if (extractionWound) extractionWound.checked = !!state.extractionWound;
  
  const extractionPlan = $("#extractionPlan");
  if (extractionPlan) extractionPlan.checked = !!state.extractionPlan;
  
  const parapulpalPin = $("#parapulpalPin");
  if (parapulpalPin) parapulpalPin.checked = !!state.parapulpalPin;
  
  const crownReplace = $("#crownReplace");
  if (crownReplace) crownReplace.checked = !!state.crownReplace;
  
  const crownNeeded = $("#crownNeeded");
  if (crownNeeded) crownNeeded.checked = !!state.crownNeeded;
  
  const missingClosed = $("#missingClosed");
  if (missingClosed) missingClosed.checked = !!state.missingClosed;
  
  const bridgePillar = $("#bridgePillar");
  if (bridgePillar) bridgePillar.checked = !!state.bridgePillar;
  
  const bridgeUnitSelect = $("#bridgeUnitSelect");
  if (bridgeUnitSelect) bridgeUnitSelect.value = state.bridgeUnit;
  ...
  if (toothSelect) toothSelect.value = state.toothSelection;
  
  const crownSelect = $("#crownSelect");
  if (crownSelect) {
  ...
  const fillingSelect = $("#fillingSelect");
  if (fillingSelect) {
  ...
  const mobilitySelect = $("#mobilitySelect");
  if (mobilitySelect) {
  ...
  const fillingSurfaceChecks = $("#fillingSurfaceChecks");
  if (fillingSurfaceChecks) fillingSurfaceChecks.classList.toggle("hidden", !showFillingSurfaces);
  ...
  const cariesSection = $("#cariesSection");
  if (cariesSection) cariesSection.classList.toggle("hidden", hideByBase || hideByRadix);
  
  const endoSection = $("#endoSection");
  if (endoSection) endoSection.classList.toggle("hidden", hideByBase);
  
  const hideFillingsByCrown = state.toothSelection === "tooth-base" && hasCrown && state.crownMaterial !== "radix";
  const fillingSection = $("#fillingSection");
  if (fillingSection) fillingSection.classList.toggle("hidden", hideByBase || hideFillingsByCrown);
  
  const hideCrownRow = hideByNone || isMilktooth || underGum || extraction;
  const crownRow = $("#crownRow");
  if (crownRow) crownRow.classList.toggle("hidden", hideCrownRow);
  
  const brokenCrownRow = $("#brokenCrownRow");
  if (brokenCrownRow) brokenCrownRow.classList.toggle("hidden", state.crownMaterial !== "broken" || hideCrownRow);
  
  const extractionRow = $("#extractionRow");
  if (extractionRow) extractionRow.classList.toggle("hidden", state.toothSelection !== "none");
  
  const inflammationSection = $("#inflammationSection");
  if (inflammationSection) inflammationSection.classList.toggle("hidden", hideByNone);
  ...
  const contactPointRow = $("#contactPointRow");
  if (contactPointRow) contactPointRow.classList.toggle("hidden", !contactAllowed);
  
  const bruxismRow = $("#bruxismRow");
  if (bruxismRow) bruxismRow.classList.toggle("hidden", !bruxismAllowed);
  
  const fissureSealingRow = $("#fissureSealingRow");
  if (fissureSealingRow) fissureSealingRow.classList.toggle("hidden", !fissureAllowed);
  ...
  const crownReplaceRow = $("#crownReplaceRow");
  if (crownReplaceRow) crownReplaceRow.classList.toggle("hidden", !crownReplaceAllowed);
  ...
  const crownNeededRow = $("#crownNeededRow");
  if (crownNeededRow) crownNeededRow.classList.toggle("hidden", !crownNeededAllowed);
  ...
  const missingClosedRow = $("#missingClosedRow");
  if (missingClosedRow) missingClosedRow.classList.toggle("hidden", !missingClosedAllowed);
  
  const bridgeUnitRow = $("#bridgeUnitRow");
  if (bridgeUnitRow) bridgeUnitRow.classList.toggle("hidden", state.toothSelection !== "none");
  
  const crownRowHidden = crownRow ? crownRow.classList.contains("hidden") : false;
  ...
  const bridgePillarRow = $("#bridgePillarRow");
  if (bridgePillarRow) bridgePillarRow.classList.toggle("hidden", !bridgePillarAllowed);
  ...
  if (mobilityRow) mobilityRow.classList.toggle("hidden", underGum || extraction);
...
function setControlsEnabled(enabled: Any){
  $$(".odon-panel-body input, .odon-panel-body select").forEach(el => {
...
function isTileNavigable(toothNo: number): boolean{
  ...
  return tiles.some(
    t => !t.classList.contains("odon-wisdom-hidden") && !t.classList.contains("placeholder") && !t.classList.contains("odon-placeholder")
  );
}
...
function updateToothTileVisibility(){
  ...
  for(const toothNo of ALL_TEETH){
    ...
    for(const tile of tiles){
      ...
      tile.classList.toggle("odon-wisdom-hidden", hide);
  ...
  selectedTeeth = new Set([...selectedTeeth].filter(tn => {
    ...
    return !tiles.every(tile => tile.classList.contains("odon-wisdom-hidden"));
  }));
  if(activeTooth && !selectedTeeth.has(activeTooth)){
    activeTooth = selectedTeeth.values().next().value ?? null;
  }
...
function setOcclusalVisible(on: Any){
  ...
  $$(".odon-tooth-tile.odon-occl-view, .odon-tooth-tile.occl-view, .tooth-tile.occl-view").forEach(tile => {
    tile.classList.toggle("occl-hidden", !occlusalVisible);
    tile.classList.toggle("odon-occl-hidden", !occlusalVisible);
  });
}
...
export function setFullStatus(data: any){
  importStatus(data);
}
...
async function loadSvg(url: Any){
  try {
    ...
    console.error(`[Odontogram] Failed to load SVG: ${url}`, e);
    return null;
  }
}
...
async function buildGrid(token: number){
  ...
  function addTile({toothNo, tplNo, rot, mirror, view, clickable}: Any){
    ...
    const svgContainer = $(".odon-tooth-svg", tile);
    if(svgContainer && svg) svgContainer.appendChild(svg);
  ...
  function addLabelRow(rowTeeth: Any, targetMap: Any){
    const row = el("div", { class:"odon-tooth-label-row", "aria-hidden":"true" });
    for(const toothNo of rowTeeth){
      const cell = el("div", { class:"odon-tooth-label-cell", text: toLabel(toothNo, numberingSystem), tabindex:"-1" });
...
function wireControls(){
  ...
  bind("#contactMesial", "change", (e)=>{
    applyToSelected((s)=>{
      s.contactMesial = (e.target as HTMLInputElement).checked;
    });
  });
  bind("#contactDistal", "change", (e)=>{
    applyToSelected((s)=>{
  ...
  bind("#btnPrimaryDentition", "click", ()=>{
    setEdentulous(false);
    suppressEdentulousSync = true;
    for(const toothNo of ALL_TEETH){
      const s = defaultState();
  ...
  bind("#btnSelectAllMissing", "click", ()=>{
...
export function setReadOnly(value: boolean){
  ...
  const panel = $(".odon-panel") as HTMLElement | null;
  if(panel) panel.classList.toggle("read-only", readOnly);
  // Update tabindex on all navigable tiles
  $$(".odon-tooth-tile[role='option'], .tooth-tile[role='option']").forEach(tile => {
    tile.setAttribute("tabindex", readOnly ? "-1" : "0");
  });
}
